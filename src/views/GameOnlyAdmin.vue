...
export default {
  ...
  methods: {
    ...
    r_selectChar(charId) {
      ...
      if (charId === 'm1') { p.className = 'นักรบดำ'; p.hp=150; p.maxHp=150; p.speed=2.5; p.damage=24; p.atkSpeed=500; p.weaponType='melee'; p.attackRange=45; p.skillName = 'สับพิฆาต'; p.skillCD = 8; }
      else if (charId === 'm2') { p.className = 'นายพราน'; p.hp=100; p.maxHp=100; p.speed=3.5; p.damage=40; p.atkSpeed=350; p.weaponType='ranged'; p.attackRange=250; p.skillName = 'ม้วนตัวพริ้ว'; p.skillCD = 5; }
      else if (charId === 'm3') { p.className = 'นักเวทย์พรต'; p.hp=80; p.maxHp=80; p.speed=2.5; p.damage=28; p.atkSpeed=600; p.weaponType='ranged'; p.attackRange=200; p.skillName = 'คาถาระเบิด'; p.skillCD = 12; }
      else if (charId === 'f1') { p.className = 'นินจาสาว'; p.hp=110; p.maxHp=110; p.speed=3.8; p.damage=24; p.atkSpeed=400; p.weaponType='melee'; p.attackRange=45; p.skillName = 'วาร์ปมีดบิน'; p.skillCD = 7; }
      else if (charId === 'f2') { p.className = 'นักบวชหญิง'; p.hp=120; p.maxHp=120; p.speed=3.0; p.damage=22; p.atkSpeed=550; p.weaponType='ranged'; p.attackRange=250; p.skillName = 'รัศมีฮีล'; p.skillCD = 15; }
      else if (charId === 'f3') { p.className = 'นักล่าปืน'; p.hp=95; p.maxHp=95; p.speed=3.5; p.damage=25; p.atkSpeed=300; p.weaponType='ranged'; p.attackRange=350; p.skillName = 'กราดยิงลูกซอง'; p.skillCD = 10; }
